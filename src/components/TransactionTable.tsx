...
import { ArrowUpDown, Search, Filter, ExternalLink, ArrowUp, ArrowDown } from 'lucide-react';
...
export const TransactionTable: React.FC<TransactionTableProps> = ({
  ...
  const filteredAndSortedTransactions = useMemo(() => {
    let filtered = transactions.filter(tx => {
      ...
      return matchesSearch && matchesType && matchesClassification && matchesDirection;
    });
    ...
    return filtered;
  }, [transactions, searchTerm, typeFilter, classificationFilter, directionFilter, sortField, sortDirection]);
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'transfer': return 'bg-purple-500/20 text-purple-200 border-purple-500/30';
      case 'trade': return 'bg-blue-500/20 text-blue-200 border-blue-500/30';
      case 'staking': return 'bg-emerald-500/20 text-emerald-200 border-emerald-500/30';
      case 'swap': return 'bg-orange-500/20 text-orange-200 border-orange-500/30';
      case 'defi': return 'bg-cyan-500/20 text-cyan-200 border-cyan-500/30';
      ...
      <Card className="bg-card border-border">
        <div className="p-6">
          <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between mb-4">
            <h3 className="text-lg font-sans font-semibold text-white">Transaction History</h3>
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <span>Showing {startIndex + 1}-{Math.min(startIndex + itemsPerPage, filteredAndSortedTransactions.length)} of {filteredAndSortedTransactions.length} transactions</span>
            </div>
          </div>
                ...
                <TableRow key={tx.id} className="border-border hover:bg-muted/30 transition-colors">
                  ...
                  <TableCell>
                    <div className="flex items-center">
                      ...
                      <span className="text-xs text-muted-foreground capitalize">{tx.direction}</span>
                    </div>
                  </TableCell>
                  ...
                  <TableCell className="font-mono text-xs">{tx.token}</TableCell>
                  <TableCell className="text-right font-mono">
                    {tx.price ? `$${(tx.amount * tx.price).toLocaleString('en-US', { 
                      minimumFractionDigits: 2, 
                  ...
                  <TableCell className="text-right font-mono">
                    {tx.gainLoss !== undefined ? (
                      <span className={tx.gainLoss >= 0 ? 'amount-positive' : 'amount-negative'}>
